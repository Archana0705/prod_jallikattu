...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    SET
        Request_status = :REQUEST_STATUS ,ASSSITANT_REMARKS=:P62_ASSSITANT_REMARKS,forward_to=:forward
    WHERE
        EVENT_ID = :P62_EVENT_ID;";
    $bind = $jk_write_db->prepare($sql);
    $bind->bindParam(':REQUEST_STATUS', $R_status);
    $bind->bindParam(':forward', $fwdTo);
    $bind->bindParam(':P62_ASSSITANT_REMARKS', $comments);
    $bind->bindParam(':P62_EVENT_ID', $event_id);
    if ($bind->execute()) {
