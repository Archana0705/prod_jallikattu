...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = " UPDATE TNEA_JALLIKATTU_EVENT_REGISTERATION_T SET Request_status = :REQUEST_STATUS ,forward_to= :forward,DC_COMMENTS=:P73_DC_COMMENTS WHERE EVENT_ID = :P73_EVENT_ID;";

    $bind = $jk_write_db->prepare($sql);
    $bind->bindParam(':REQUEST_STATUS', $R_status);
    $bind->bindParam(':forward', $fwdTo);
    $bind->bindParam(':P73_DC_COMMENTS', $comments);
    $bind->bindParam(':P73_EVENT_ID', $event_id);
    if ($bind->execute()) {
