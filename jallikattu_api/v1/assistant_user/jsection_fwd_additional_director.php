...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $R_status = 'F5';
    $fwdTo = 'Additional Director';
    $sql = "UPDATE TNEA_JALLIKATTU_EVENT_REGISTERATION_T
    SET
        Request_status = :REQUEST_STATUS,forward_to= :forward,JOINT_REMARKS=:P19_JOINT_REMARKS,updated_by=:app_employee_id
    WHERE
        EVENT_ID = :P19_EVENT_ID;";
    $bind = $jk_write_db->prepare($sql);
    $bind->bindParam(':REQUEST_STATUS', $R_status);
    $bind->bindParam(':forward', $fwdTo);
    $bind->bindParam(':P19_JOINT_REMARKS', $comments);
