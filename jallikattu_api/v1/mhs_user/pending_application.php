...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $sql = "SELECT distinct e.EVENT_ID, e.EVENT_DATE, e.NAME_OF_ORGANIZATION, e.NAME_OF_PRESIDENT, e.CONTACT_OF_PRESIDENT, e.MOBILE_OF_PRESIDENT, e.NAME_OF_VILLAGE, e.NAME_OF_TALUK, e.NAME_OF_DISTRICT, e.PLACE_OF_EVENT, CASE WHEN e.REQUEST_status = 'E' THEN 'Save' WHEN e.REQUEST_status = 'S' THEN 'Pending for Approval' WHEN e.REQUEST_status = 'C' THEN 'Closed' WHEN e.REQUEST_status = 'D1' THEN 'District Collector Approved for Inspection' WHEN e.REQUEST_status = 'D2' THEN 'Pre-Inspection Completed, Waiting for District Collector Approval' WHEN e.REQUEST_status = 'DNN' THEN 'Need more Information - District Collector' WHEN e.REQUEST_status = 'N' THEN 'Need more Information' WHEN e.REQUEST_status = 'A' THEN 'Need to upload GO from Secretariat' WHEN e.REQUEST_status = 'A1' THEN 'Waiting for District Collector Approval' WHEN e.REQUEST_status = 'A2' THEN 'Waiting for DAH Approval' WHEN e.REQUEST_status IN ('F1', 'F2', 'F3', 'F4', 'F5', 'F6') THEN 'Waiting for DAH Approval' WHEN e.REQUEST_status = 'GO1' THEN 'GO Issued from Secretariat' WHEN e.REQUEST_status = 'GO2' THEN 'GO Issued, Waiting for Final/Pre-Event Inspection' WHEN e.REQUEST_status = 'GO3' THEN 'GO Issued, Final/Pre-Event Inspection Completed, Waiting for DC Approval' WHEN e.REQUEST_status = 'A3' THEN 'Waiting for Principal Secretary Approval' WHEN e.REQUEST_status = 'A4' THEN 'Principal Secretary Approved' WHEN e.REQUEST_status IN ('DN', 'AN', 'SN', 'N2', 'N1', 'N3', 'MN') THEN 'Need more Information' WHEN e.REQUEST_status = 'RO' THEN 'Application Resubmitted from Event Organizer' WHEN e.REQUEST_status = 'R1' THEN 'DC Rejected Based on JMC Report' WHEN e.REQUEST_status = 'RM1' THEN 'REJECTED BY MHS' WHEN e.REQUEST_status = 'R11' THEN 'DC Not permitted the Event' WHEN e.REQUEST_status IN ('R2', 'R3', 'R4') THEN 'Rejected' END AS REQUEST_status, e.REQUEST_LETTER, e.ATTACH_FILENAME, e.ATTACH_MIMETYPE, e.ASSURANCE_BOND, e.ASSURANCE_BOND_ATTACH_FILENAME, e.ASSURANCE_BOND_ATTACH_MIMETYPE, e.INSURANCE_COPY, e.INSURANCE_COPY_ATTACH_FILENAME,  e.INSURANCE_COPY_ATTACH_MIMETYPE, e.LAYOUT_SKETCH, e.LAYOUT_SKETCH_ATTACH_FILENAME, e.LAYOUT_SKETCH_ATTACH_MIMETYPE, e.OTHER_DOCS, e.OTHER_DOCS_ATTACH_FILENAME, e.OTHER_DOCS_ATTACH_MIMETYPE, e.PREVIOUS_EVENT, e.PREVIOUS_EVENT_ATTACH_FILENAME, e.PREVIOUS_EVENT_ATTACH_MIMETYPE, e.PANCHAYAT_UNION, e.PANCHAYAT_UNION_ATTACH_FILENAME, e.PANCHAYAT_UNION_ATTACH_MIMETYPE, e.FIR, e.FIR_ATTACH_FILENAME, e.FIR_ATTACH_MIMETYPE, e.UPLOAD_PHOTO, e.UPLOAD_PHOTO_ATTACH_FILENAME, e.UPLOAD_PHOTO_ATTACH_MIMETYPE, a.name, d.ARENA_UPLOAD, d.ARENA_UPLOAD_FILENAME, d.ARENA_UPLOAD_MIME_TYPE, d.LENGTH_ARENA_UPLOAD, d.BULL_ARENA_UPLOAD, d.BULL_ARENA_UPLOAD_FILENAME, d.BULL_ARENA_UPLOAD_MIME_TYPE, d.BULL_EXAMINATION_UPLOAD, d.BULL_EXAMINATION_UPLOAD_FILENAME, d.BULL_EXAMINATION_UPLOAD_MIME_TYPE from TNEA_JALLIKATTU_EVENT_REGISTERATION_T e,TNEA_JALLIKATTU_APPROVER_LOGIN_T a,TNEA_JALLIKATTU_DISTRICT_LEVEL_MONITORING_REGISTERATION_T D where TRIM(a.role) = 'Magesterial huzur sharishthadhar MHS'AND UPPER(e.NAME_OF_DISTRICT) = UPPER(A.DISTRICT) AND E.EVENT_ID = D.EVENT_ID and e.request_status in ('S','RO','SN') AND A.APPROVER_ID=:APP_EMPLOYEE_ID order by e.event_date asc;";
