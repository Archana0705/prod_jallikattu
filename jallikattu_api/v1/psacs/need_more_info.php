...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    SET
        Request_status = :REQUEST_STATUS, forward_to=:forward,ACS_PS_COMENTS=:P20_ACS_PS_COMENTS,UPDATED_BY=:APP_EMPLOYEE_ID
    WHERE
        EVENT_ID = :P20_EVENT_ID;";
    $bind = $jk_write_db->prepare($sql);
    $bind->bindParam(':REQUEST_STATUS', $R_status);
    $bind->bindParam(':forward', $R_status);
    $bind->bindParam(':P20_ACS_PS_COMENTS', $comments);
