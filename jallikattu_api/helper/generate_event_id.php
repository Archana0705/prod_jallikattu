...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checksql = "SELECT register_id from tnea_jallikattu_register_login_t where register_id = :user_id";
    $bind = $jk_read_db->prepare($checksql);
    $bind->bindParam(':user_id', $app_employee_id, PDO::PARAM_STR);
    if($bind->execute()){
        $result = $bind->fetchAll(PDO::FETCH_ASSOC);
        if($result){
            $sql = "SELECT SUBSTRING(UPPER(:district), 1, 3) || '/' || TO_CHAR(CURRENT_DATE, 'YYMMDD') || '/' || 'N' || '/' || SUBSTRING(UPPER(:event_name),1,3)  || '/' || LPAD(NEXTVAL('TNEA_JALLIKATTU_EVENT_REGISTER_SEQ')::text, 5, '0') AS generated_event_id;";
